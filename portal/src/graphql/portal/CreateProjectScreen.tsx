...
import { randomProjectName, getRandom32BitsNumber, maskNumber } from "../../util/projectname";
...
function addRandomNumberSuffix(intermediateProjectName: string): string {
  return intermediateProjectName + "-" + maskNumber(getRandom32BitsNumber(), 0, 10).toString();
}
...
function CreateProjectScreenContent(props: CreateProjectScreenContentProps) {
  ...
  const defaultState = useMemo(() => {
    ...
    if (typedState) {
      ...
      if (intermediateName !== "") defaultState.appID = addRandomNumberSuffix(intermediateName)
    }
    return defaultState;
  }, [state]);
